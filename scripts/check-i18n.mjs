...
const ru = JSON.parse(fs.readFileSync(path.join(root, "src/lib/locales/ru.json"), "utf-8"));
const en = JSON.parse(fs.readFileSync(path.join(root, "src/lib/locales/en.json"), "utf-8"));
const zh = JSON.parse(fs.readFileSync(path.join(root, "src/lib/locales/zh.json"), "utf-8"));

// Walk through files and find t("key") / t('key') patterns
...
// Check keys that are in locale files but not statically referenced anymore
// (dynamic keys like t(cat.labelKey) will show up here — expected)
